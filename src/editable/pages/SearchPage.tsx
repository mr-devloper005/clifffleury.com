...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  const resolvedLabel = SITE_CONFIG.tasks.find((item) => item.key === task)?.label || 'Post'
  const taskLabel = task && hiddenTaskKeys.has(task) ? 'Post' : resolvedLabel
  const strong = index % 5 === 0

  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 rounded-full bg-[var(--slot4-cream)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--slot4-page-bg)]">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-[var(--editable-cta-bg)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--slot4-on-accent)]">{taskLabel}</span> : null}
        <h2 className="mt-4 line-clamp-3 text-2xl font-black leading-[0.95] tracking-[-0.06em] text-[var(--editable-page-text)]">{post.title}</h2>
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  const enabledTasks = SITE_CONFIG.tasks.filter((item) => item.enabled && !hiddenTaskKeys.has(item.key))

  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg)] text-[var(--editable-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-[var(--editable-border)] bg-[rgba(29,84,109,0.22)] p-6 shadow-[0_30px_90px_rgba(6,30,41,0.28)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-[var(--editable-border)] bg-[var(--editable-page-bg)] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--editable-cta-bg)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--slot4-on-accent)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/article" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-black">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-[rgba(29,84,109,0.22)] p-10 text-center">
